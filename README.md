...
- gateTwo: to see the remaining gas we can use the functionality of Remix-IDE. The value we are looking for will be shown after the opcode GAS, i.e. in the opcode PUSH2 (see picture below). Knowing this value we could add the proper gas amount to our call in order to pass gateTwo. However, as mentioned by Spalladino, the proper gas offset to use will vary depending on the compiler. We will use [his solution](https://github.com/OpenZeppelin/ethernaut/blob/solidity-05/contracts/attacks/GatekeeperOneAttack.sol) in our malicious contract and brute-force a range of possible gas values. To account for it, we will increase the gas limit in Remix-IDE.<br/>
...
*await contract.increaseAllowance(player, "1000000000000000000000000")*<br/>
*await contract.transferFrom (player, "another_address" , "1000000000000000000000000")* // replace another_address by another address -_-<br/>
*(await contract.balanceOf(player)).toString()*<br/>
...
\* to compute the address of the contract you need to know the creator’s address (in our case the instance address) and the nonce (in this case 1, as this contract’s creation is its first transaction). In the console use the command: *web3.utils.soliditySha3("0xd6", "0x94", instance_address, “0x01”)*, the address will be the first 20 bytes (=160 bits). For more detail, please have a look at the [RLP documentation](https://github.com/ethereum/wiki/wiki/RLP) or this [post](https://ethereum.stackexchange.com/questions/24248/how-to-calculate-an-ethereum-contracts-address-during-its-creation-using-the-so).
